...
public class ChessBoardImpl implements ChessBoard {
            ...
            for(int j = 0; j < 8; j++){
                ...
                if(chessBoard.getPiece(currentPosition) != null){
                    switch(chessBoard.getPiece(currentPosition).getPieceType()){
                        case PAWN -> this.addPiece(currentPosition, new PawnImpl((PawnImpl) chessBoard.getPiece(currentPosition)));
                        case ROOK -> this.addPiece(currentPosition, new RookImpl((RookImpl) chessBoard.getPiece(currentPosition)));
                        case KNIGHT -> this.addPiece(currentPosition,new KnightImpl((KnightImpl) chessBoard.getPiece(currentPosition)));
                        case KING -> this.addPiece(currentPosition, new KingImpl((KingImpl) chessBoard.getPiece(currentPosition)));
                        case BISHOP -> this.addPiece(currentPosition, new BishopImpl((BishopImpl) chessBoard.getPiece(currentPosition)));
                        case QUEEN -> this.addPiece(currentPosition, new QueenImpl((QueenImpl) chessBoard.getPiece(currentPosition)));
                    }
                }
            }
        }
    ...
    public void addPiece(ChessPosition position, ChessPiece piece) {
        switch (piece.getPieceType()) {
            ...
            case BISHOP -> board[(position.getRow()-1)][(position.getColumn()-1)] = new BishopImpl((BishopImpl) piece);
            case KNIGHT -> board[(position.getRow()-1)][(position.getColumn()-1)] = new KnightImpl((KnightImpl) piece);
